...
(source, el, ctx) => {
    ...
    if (matches) {
        ...
        /*
        var [widths, heights] = await measureSizes(htmls);
        for (var i = 0; i < matches.length; i++) {
            var match = matches[i];
            var html = htmls[i];
            var width = widths[i];
            var height = heights[i];
            var id = html.replace(/[^a-zA-Z0-9]/g, "") + i;
            source = source.replace(match, `<div class="${graph_id}${id}" style="width: ${width}px; height: ${height}px; display: inline-block;"></div>`);
        }
        const { svg } = await mermaid.render(graph_id, source);
        el.innerHTML = svg;
        for (var i = 0; i < htmls.length; i++) {
            var id = htmls[i].replace(/[^a-zA-Z0-9]/g, "") + i;
            var htmlEl = document.getElementsByClassName(graph_id + id)[0];
            htmlEl.innerHTML = htmls[i];
        }
        */
    }
});